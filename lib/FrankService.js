...
class FrankService {
    ...
    async getPrices(start, end) {
        for (let i = 0; i < FrankService.Retries; i++) {
            try {
                ...
                await (0, Util_1.Sleep)(this._app, FrankService.RetryTimeoutMs);
            }
        }
    ...
    async getPricesImpl(start, end) {
        ...
        this._app.log(`Calling frank-energie API to get prices between ${startDate} and ${endDate}`);
        ...
        this._app.log(`API call took ${duration}ms`);
        if (body.errors) {
            // this._app.error(body.errors);
            throw new Error("Failed to get prices; " + body.errors[0].message);
        }
        this._app.log(`Got ${body.data.marketPricesElectricity.length} electricity prices and ${body.data.marketPricesGas.length} gas prices`);
        return body.data;
    }
}
